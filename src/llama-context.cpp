...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    do {
        ...
        for (const auto & [seq_id, t_probs] : res->t_sampled_probs) {
            ...
            sampled_probs_map[idx].resize(n_vocab);
            ggml_backend_tensor_get_async(backend, t_probs, sampled_probs_map[idx].data(), 0, n_vocab * sizeof(float));
        }

        gpu_has_sampled = !res->t_sampled_tokens.empty() || !res->t_sampled_probs.empty();
