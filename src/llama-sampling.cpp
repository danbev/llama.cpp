...
llama_token llama_sampler_sample(struct llama_sampler * smpl, struct llama_context * ctx, int32_t idx) {
    const llama_token gpu_selected_token = llama_get_sampled_token_ith(ctx, idx);
    if (gpu_selected_token != LLAMA_TOKEN_NULL) {
        return gpu_selected_token;
    }

    const auto * probs  = llama_get_sampled_probs_ith(ctx, idx);
    ...
    for (llama_token token_id = 0; token_id < n_vocab; token_id++) {
        if (probs) {
            cur.emplace_back(llama_token_data{token_id, 0.0f, probs[token_id]});
        } else {
